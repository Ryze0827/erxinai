# WayX Gateway 官网原型

[English](./README.md)

WayX 是一个面向 AI 中转服务的响应式官网原型。它用于展示统一且兼容 OpenAI 的多模型访问入口，以及请求追踪、用量与成本统计、供应商健康状态和故障转移等能力。

当前仓库包含官网、交互式中转演示，以及与 `sub2api` 后端兼容的完整认证前端。AI 中转控制台仍为前端演示。

## 主要功能

- 适配桌面端和移动端的响应式首页
- AI 中转控制台交互演示
- OpenAI 兼容接口展示
- 请求链路、状态和延迟展示
- Token 用量和成本监控概念展示
- 模型供应商健康状态和自动故障转移概念展示
- 密码登录、邮箱验证、2FA、密码找回和 OAuth 认证闭环
- 定价、FAQ 和导航交互
- 本地字体和图片资源

## 当前范围

本工程由静态中转演示和真实认证客户端组成：

- 认证统一访问 `/api/v1`，支持 Token 刷新及 OAuth pending-session Cookie。
- AI 中转控制台使用演示数据，不会请求后端接口。
- 完整认证需要运行兼容的后端，并配置数据库及第三方认证提供方。

## 技术栈

- React 19
- Vite 8
- React Router
- React Markdown
- JavaScript 和 CSS

## 本地开发

当前安全修复后的依赖要求使用 Node.js 22.22.0 或更高版本。

```bash
npm install
npm run dev
```

开发服务器默认监听所有本地网络接口，将 `/api` 代理到 `http://127.0.0.1:8080`，并通过同源路径 `/image-api` 代理生图网关，避免本地开发时触发浏览器跨域限制。

可选环境变量：

```bash
VITE_API_BASE_URL=/api/v1
VITE_DEV_API_PROXY_TARGET=http://127.0.0.1:8080
VITE_DEV_IMAGE_PROXY_TARGET=https://image.aiwayxx.com
VITE_DASHBOARD_URL=/dashboard
```

## 可用命令

```bash
npm run dev      # 启动 Vite 开发服务器
npm run build    # 构建生产文件到 dist/ 目录
npm run preview  # 在本地预览生产构建
```

## 工程结构

```text
.
├── index.html                       # Vite HTML 入口和页面元信息
├── src/
│   ├── App.jsx                      # 首页行为和应用路由
│   ├── api/                         # API 客户端、认证接口和 Token 存储
│   ├── auth/                        # 认证页面、通用控件和 OAuth 状态机
│   ├── gatewayDemos.js              # AI 中转交互演示
│   ├── landing-page.html            # 首页主体 HTML
│   └── *.css                        # 页面和组件样式
├── public/assets/                   # 本地字体和图片
└── vite.config.mjs                  # Vite 配置
```

## 页面路由

| 路由 | 用途 |
| --- | --- |
| `/` | WayX 中转服务首页 |
| `/login`、`/register`、`/email-verify` | 密码登录和注册流程 |
| `/forgot-password`、`/reset-password` | 密码找回流程 |
| `/auth/success` | 统一认证成功页 |
| `/auth/*/callback` | OAuth 回调和账号补全流程 |

页面路由由 React 应用在客户端处理。由于构建结果中没有顶层 `404.html`，Cloudflare Pages 可以通过默认的单页应用回退机制访问这些路由。

## 部署到 Cloudflare Pages

请使用以下构建配置：

| 配置项 | 填写内容 |
| --- | --- |
| 生产分支 | `main` |
| 框架预设 | `React (Vite)` |
| 构建命令 | `npm run build` |
| 构建输出目录 | `dist` |
| 根目录 | 留空 |

生产环境应保持 `/api/v1` 同源，并配置后端 `server.frontend_url`、所有已启用 OAuth 的 backend callback/frontend redirect，以及认证路由的 SPA fallback。

## 正式上线前

- 将中转控制台接入真实的请求、用量、账单和供应商健康数据。
- 确认最终定价和 FAQ 内容，并在准备完成后补充 WayX 自有的法律条款与联系页面。
- 补充社交分享元信息并确认正式域名。
- 接入后端后补充合适的安全响应头和密钥管理方案。
