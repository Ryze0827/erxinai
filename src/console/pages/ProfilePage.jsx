...
function ProfileAvatarRow({ avatar, email, username, status, statusText, joined, onError }) {
  ...
  return <div className="console-profile-avatar">{avatar ? <img className="console-profile-avatar-mark" src={avatar} alt={displayName} onError={onError} /> : <span className="console-profile-avatar-mark">{initials}</span>}<div><strong>{displayName}</strong><small>{email}</small><div className="console-profile-status"><StatusBadge status={status || "active"} label={statusText} /></div><time>{joined}</time></div></div>;
}
...
function PasswordFields({ form, setForm, visible, setVisible, copy, t }) {
  ...
  const visibility = () => <IconButton icon={visible ? "eyeOff" : "eye"} label={visible ? "Hide" : "Show"} onClick={() => setVisible((value) => !value)} />;
  return <div className="console-form-grid"><Field label={t("profile.currentPassword")}><div className="console-input-wrap"><TextInput type={type} value={form.old} onChange={(event) => setForm((current) => ({ ...current, old: event.target.value }))} autoComplete="current-password" />{visibility()}</div></Field><Field label={t("profile.newPassword")} hint={copy.hint}><div className="console-input-wrap"><TextInput type={type} minLength="8" value={form.next} onChange={(event) => setForm((current) => ({ ...current, next: event.target.value }))} autoComplete="new-password" />{visibility()}</div></Field><Field label={copy.confirm} error={mismatch ? copy.mismatch : ""}><div className="console-input-wrap"><TextInput type={type} minLength="8" value={form.confirm} onChange={(event) => setForm((current) => ({ ...current, confirm: event.target.value }))} autoComplete="new-password" />{visibility()}</div></Field></div>;
}
