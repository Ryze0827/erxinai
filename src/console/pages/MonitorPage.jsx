...
function StatusRow({ row, mode, showThroughput, range, locale, formatNumber, labels, updatedAt, coverage }) {
  ...
  return <>
        <TableRow highlighted={expanded} className="console-group-overview-row" data-tone={row.tone}>
          ...
          <TableCell><MetricRing metric={mode === "v2" ? "cache" : undefined} value={mode === "v2" ? row.cacheRate : row.availability} accessibleLabel={row.name + " · " + labels[showThroughput ? 5 : 4]} label={mode === "v2" ? range : localized(locale, "探测可用率", "Probe uptime")} tone={mode === "v2" ? monitorTone(row.source.health?.cache) : "operational"} /></TableCell>
