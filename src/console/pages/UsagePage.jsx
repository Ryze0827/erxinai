...
function TokenCell({ row, locale }) {
  ...
  const title = `${locale === "zh" ? "输入" : "Input"}: ${formatTokenMillions(row.input_tokens)}\n${locale === "zh" ? "输出" : "Output"}: ${formatTokenMillions(row.output_tokens)}\n${locale === "zh" ? "缓存创建" : "Cache creation"}: ${formatTokenMillions(row.cache_creation_tokens)} (5m ${formatTokenMillions(row.cache_creation_5m_tokens)}, 1h ${formatTokenMillions(row.cache_creation_1h_tokens)})\n${locale === "zh" ? "缓存命中" : "Cache hit"}: ${formatTokenMillions(row.cache_read_tokens)}\nTotal: ${formatTokenMillions(total)}`;
  return <div className="console-token-cell" title={title}><div className="console-token-metrics"><Badge variant="info" size="lg" className="console-token-metric"><b>{locale === "zh" ? "输入" : "Input"}</b><strong>{formatTokenMillions(row.input_tokens)}</strong></Badge><Badge variant="success" size="lg" className="console-token-metric"><b>{locale === "zh" ? "输出" : "Output"}</b><strong>{formatTokenMillions(row.output_tokens)}</strong></Badge><Badge variant="secondary" size="lg" className="console-token-metric is-cache-hit"><Icon name="cache" size={13} data-icon="start" /><b>{locale === "zh" ? "缓存命中" : "Cache hit"}</b><strong>{formatTokenMillions(row.cache_read_tokens)}</strong></Badge></div></div>;
}
