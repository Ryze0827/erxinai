...
function resolveWeChatStart(settings, t) {
  ...
  if (inWeChat && openEnabled) return { error: t("auth.oauth.wechatSystemBrowser") };
  if (!inWeChat && openEnabled) return { mode: "open" };
  if (!inWeChat && mpEnabled) return { error: t("auth.oauth.wechatApp") };
  return { error: t("auth.oauth.wechatUnavailable") };
}
...
export function OAuthButtons({ settings, searchParams, onError }) {
  ...
  const startOAuth = (provider) => {
    ...
    if (provider === "wechat") {
      const result = resolveWeChatStart(settings, t);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="text-foreground-subtle flex items-center gap-3 text-xs"><Separator className="flex-1" /><span>{t("auth.common.orContinueWith")}</span><Separator className="flex-1" /></div>
        ...
        {providers.map((provider) => {
          ...
          return (
            <Button className="w-full" variant="outline" type="button" key={provider} onClick={() => startOAuth(provider)}>
              ...
              <span>{singleProvider ? t("auth.common.signInWith", { provider: label }) : label}</span>
            </Button>
          );
        })}
      </div>
